...
ResultTable QueryOne::execute(const table& tab){
    ...
    return op_sort(groups);
}

ResultTable QueryOne::execute_hybrid(const table & tab)
{
    auto bitmap = op_shipdate_se(tab);
    ResultMap result;
    for (int i = 0; i < tab.l_shipdate.size(); i++) {
        std::string key = std::string() + tab.l_returnflag[i] + tab.l_linestatus[i];
        result.insert(std::pair<std::string, ResultRow>(key, ResultRow()));
...
ResultTable QueryOne::op_sort(const ResultMap & groups)
{
    ResultTable rt(groups.begin(), groups.end());
    std::sort(rt.begin(), rt.end(), [](auto& a, auto&b) {
