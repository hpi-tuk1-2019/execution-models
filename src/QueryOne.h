...
struct ResultTable {
    ...
    std::vector<int> sum_qty;
    std::vector<int> sum_base_price; // sum(l_extendedprice)
    std::vector<int> sum_disc_price; // sum(l_extendedprice * (1 - l_discount))
    std::vector<int> sum_charge; // sum(l_extendedprice * (1 - l_discount) * (1 + l_tax))
...
class QueryOne {
  private:
    ...
    std::vector<int> op_sum_qty(const table& tab, std::vector<int> old_inds, std::vector<int> groups);
    std::vector<int> op_sum_base_price(const table& tab, std::vector<int> old_inds, std::vector<int> groups);
    std::vector<int> op_sum_disk_price(const table& tab, std::vector<int> old_inds, std::vector<int> groups);
    std::vector<int> op_sum_charge(const table& tab, std::vector<int> old_inds, std::vector<int> groups);
