...
class QueryOne {
  private:
    ...
    void op_sum_disk_price(const table& tab, const std::vector<BITMAP_TYPE>& bitmap, ResultMap& groups);
    void op_sum_charge(const table& tab, const std::vector<BITMAP_TYPE>& bitmap, ResultMap& groups);
    void op_avg_qty(const table& tab, ResultMap& groups);
    void op_avg_price(const table& tab, ResultMap& groups);
    void op_avg_disc(const table& tab, const std::vector<BITMAP_TYPE>& bitmap, ResultMap& groups);
    void op_count_order(const table& tab, const std::vector<BITMAP_TYPE>& bitmap, ResultMap& groups);
  public:
